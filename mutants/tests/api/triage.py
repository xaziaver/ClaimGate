...
def route_queue(severity: str, late_reporting: bool, recent_inception: bool) -> str:
    flags = SiuFlags(late_reporting=late_reporting, recent_policy_inception=recent_inception)
    return _route_queue(severity, flags)
...
def triage_and_route(
    *,
    ...
    return _triage_and_route(candidate, now=now)
