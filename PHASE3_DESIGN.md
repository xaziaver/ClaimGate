...
## What the code was measured to do when this design was written

Moved on 2026-09-11, clean-up stage C2d, to
`docs/queue-history/phase-3-design-measurements.md`, unedited. It held six measurements of the
phase-2 shell taken at `4a42d2f` on 2026-09-01, and four later annotations — items 7f, 7g and 7h —
each overturning part of the list above it. By `prototype-1` every measured claim had been
superseded by the work this design scheduled, so the section described neither the code nor a
decision. It is kept because the decisions below were made against those measurements, and a reader
asking why a port is shaped as it is may need to see what the shell looked like at the time.

For what the code does now, read the code. The behaviour of the harness around it is
`docs/harness-findings.md`; the decisions in force are `ASSUMPTIONS.md`.
