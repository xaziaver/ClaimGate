# Work queue history — phases 1 and 2

Moved verbatim from `QUEUE.md` on 2026-09-11, clean-up stage C2a, and not to be edited: items 1–5j,
then the status log from 2026-08-09 through the paragraph that closed the queue at 5j. Phases 1 and
2 ran as one numbered queue with one interleaved log, and items were renumbered across them, so they
are kept together. Every present-tense statement is true as of its paragraph's date. The live queue
is `QUEUE.md`.
